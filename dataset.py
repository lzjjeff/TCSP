...
class MoseiDataset(Dataset):
    def __init__(self, data_dic, dname="train", v_max=1.0, a_max=1.0):
    ...
    def __getitem__(self, idx):
        ...
        test = np.sum(np.isnan(audio_normed))
        if test != 0:
            print(f"======================DATA ERROR:{test}======================")

        video = self.data_dic[key]['v']
        video[~np.isfinite(video)] = 0
        video_normed = video / self.v_max

        test = np.sum(np.isnan(video_normed))
        if test != 0:
            print(f"======================DATA ERROR:{test}======================")

        data = {"text": self.data_dic[key]['t'], "video": video,
